 ...
 *  A null `expr` pauses the query (SWR skips null keys) — for data a panel only
 *  needs in one of its states, so it is not polled every 15s regardless. */
export function useInstantVector(expr: string | null): InstantResult {
  const { data, isLoading, error } = useSWR<PromInstantResponse>(
    expr === null ? null : instantKey(expr),
    promFetcher,
    SWR_OPTIONS,
  );
