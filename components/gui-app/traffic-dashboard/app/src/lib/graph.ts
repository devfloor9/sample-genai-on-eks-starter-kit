...
/** What a node stands for. The service map is all services; the pod drill-down
 *  mixes services (the neighbours) with the focused service's pods, or AZ groups
 *  of them when there are too many to draw one by one. */
export type NodeKind = "service" | "pod" | "group";

export interface NodeMeta {
  kind: NodeKind;
  label: string;
  /** Second line under the label (AZ for a pod, pod count for a group). */
  sublabel?: string;
}

/** Where an edge's numbers come from — shown in the tooltip so an estimate is
 *  never mistaken for a measurement. */
export type EdgeOrigin =
  /** Tempo paired the client and server spans of the same requests: pod-exact. */
  | "tempo"
  /** Beyla's client-side RED on the calling pod: exact rate, no error/latency. */
  | "beyla-client"
  /** Service-level total split across pods in proportion to their inbound rate. */
  | "estimate";
...
export interface EdgeInput {
  ...
  /** Portion of `rate` whose two endpoints sit in different AZs; undefined when
   *  the endpoints' placement is unknown (service-level edges without a Tempo
   *  pairing behind them). */
  crossAzRate?: number;
  origin?: EdgeOrigin;
  clientMeta?: NodeMeta;
...
export interface GraphEdge extends EdgeInput {
  ...
  errorRatio: number;
  health: StatusLevel;
  /** Share of requests crossing an AZ boundary, 0..1; null when unknown. */
  crossAzShare: number | null;
...
export function layoutServiceGraph(inputs: EdgeInput[]): GraphLayout {
  ...
  // Optional display metadata travels on the edges; the first mention wins.
  const metaById = new Map<string, NodeMeta>();
  for (const edge of edges) {
    if (edge.clientMeta && !metaById.has(edge.client)) metaById.set(edge.client, edge.clientMeta);
    if (edge.serverMeta && !metaById.has(edge.server)) metaById.set(edge.server, edge.serverMeta);
  }
  ...
  rows.forEach((row, layer) => {
    ...
    row.forEach((id, index) => {
      nodes.push({
        id,
        label: metaById.get(id)?.label ?? shortLabel(id),
        sublabel: metaById.get(id)?.sublabel,
        kind: metaById.get(id)?.kind ?? "service",
  ...
  const laidOut: GraphEdge[] = edges
    ...
    .map((edge) => {
      ...
      const crossAzShare =
        edge.crossAzRate !== undefined && edge.rate > 0 ? Math.min(edge.crossAzRate / edge.rate, 1) : null;
      return {
        ...edge,
...
function dedupe(inputs: EdgeInput[]): EdgeInput[] {
  ...
  for (const input of inputs) {
    ...
    if (input.crossAzRate !== undefined) {
      existing.crossAzRate = (existing.crossAzRate ?? 0) + input.crossAzRate;
    }
    existing.clientMeta ??= input.clientMeta;
    existing.serverMeta ??= input.serverMeta;
