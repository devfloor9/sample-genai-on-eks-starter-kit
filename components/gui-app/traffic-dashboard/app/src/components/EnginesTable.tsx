...
type EngineSortKey =
  | "model" | "namespace" | "pod" | "node" | "pods" | "accelerator" | "devices" | "accelUtil" | "accelMem"
  | "kvUsage" | "prefixHit" | "running" | "waiting" | "genTps" | "ttftP95" | "tenants";

const hitRatio = (r: { cacheHits: number | null; cacheQueries: number | null }) =>
  r.cacheQueries === null || r.cacheQueries <= 0 ? null : (r.cacheHits ?? 0) / r.cacheQueries;

/** Metrics shared by both groupings; the grouping-specific identity columns are added below. */
const METRIC_COLUMNS = <R extends EngineRow | ModelRow>(): SortColumn<R, EngineSortKey>[] => [
  strCol<R, EngineSortKey>("model", (r) => r.model),
  numCol<R, EngineSortKey>("devices", (r) => r.devices),
  numCol<R, EngineSortKey>("accelUtil", (r) => r.accelUtil),
  numCol<R, EngineSortKey>("accelMem", (r) => r.accelMem),
  numCol<R, EngineSortKey>("kvUsage", (r) => r.kvUsage),
  numCol<R, EngineSortKey>("prefixHit", (r) => hitRatio(r)),
  numCol<R, EngineSortKey>("running", (r) => r.running),
  numCol<R, EngineSortKey>("waiting", (r) => r.waiting),
  numCol<R, EngineSortKey>("genTps", (r) => r.genTps),
  numCol<R, EngineSortKey>("ttftP95", (r) => r.ttftP95),
  strCol<R, EngineSortKey>("tenants", (r) => (r.tenants.length ? r.tenants.join(", ") : null)),
];

const MODEL_COLUMNS: SortColumn<ModelRow, EngineSortKey>[] = [
  ...METRIC_COLUMNS<ModelRow>(),
  numCol<ModelRow, EngineSortKey>("pods", (r) => r.pods),
  strCol<ModelRow, EngineSortKey>("accelerator", (r) => (r.accelerators.length ? r.accelerators.join(", ") : null)),
];

const POD_COLUMNS: SortColumn<EngineRow, EngineSortKey>[] = [
  ...METRIC_COLUMNS<EngineRow>(),
  strCol<EngineRow, EngineSortKey>("namespace", (r) => r.namespace),
  strCol<EngineRow, EngineSortKey>("pod", (r) => r.pod),
  strCol<EngineRow, EngineSortKey>("node", (r) => r.node),
  strCol<EngineRow, EngineSortKey>("accelerator", (r) => r.accelerator),
];

/** One state for both groupings: a column missing from the current grouping sorts as all-equal, so the tie-break order shows. */
const ALL_COLUMNS = [...MODEL_COLUMNS, ...POD_COLUMNS] as SortColumn<EngineRow | ModelRow, EngineSortKey>[];

/** Busiest pool first, as the table has always opened. */
const DEFAULT_SORT = { key: "genTps", dir: "desc" } as const;
const modelTieBreak = (a: ModelRow, b: ModelRow) => (b.genTps ?? -1) - (a.genTps ?? -1) || a.model.localeCompare(b.model);
const podTieBreak = (a: EngineRow, b: EngineRow) =>
  (b.genTps ?? -1) - (a.genTps ?? -1) || a.model.localeCompare(b.model) || a.pod.localeCompare(b.pod);
...
export function EnginesTable({ grouping, filter = EMPTY_FILTER }: { grouping: EnginesGrouping; filter?: AcceleratorFilter }) {
  ...
  const { sort, toggle } = useSortState<EngineSortKey>(DEFAULT_SORT, ALL_COLUMNS);
  ...
  const models = useMemo<ModelRow[]>(() => {
    ...
    return sortRows(aggregated, sort, MODEL_COLUMNS, modelTieBreak);
  }, [visible, ttftModel.rows, sort]);

  const podRows = useMemo(() => sortRows(visible, sort, POD_COLUMNS, podTieBreak), [visible, sort]);
  ...
  // Engines exist but none survived, so the filter is on by definition.
  if (visible.length === 0) {
    return (
      <p className="py-8 text-center text-xs text-ink-muted">
        No engine matches the current filters — clear a filter to widen the view.
      </p>
    );
  }
      ...
      <table className="w-full border-collapse text-xs">
        <thead>
          <tr className="border-b border-gridline text-left text-ink-muted">
            <SortableTh label="Model pool" sortKey="model" {...sortable} />
            {grouping === "pod" ? (
              <>
                <SortableTh label="Namespace" sortKey="namespace" {...sortable} />
                <SortableTh label="Pod" sortKey="pod" {...sortable} />
                <SortableTh label="Node" sortKey="node" {...sortable} />
              </>
            ) : (
              <SortableTh label="Pods" sortKey="pods" {...sortable} align="right" />
            )}
            <SortableTh label="Accelerator" sortKey="accelerator" {...sortable} />
            <SortableTh label="Devices" sortKey="devices" {...sortable} align="right" title="GPUs (DCGM) or Neuron cores (neuron-monitor) behind the engine" />
            <SortableTh
              label="Accel util"
              sortKey="accelUtil"
              {...sortable}
              title="GPU: DCGM utilisation averaged over the pod's GPUs. Neuron: core utilisation on the pod's node. Target ≥ 70%."
            />
            <SortableTh label="Accel mem" sortKey="accelMem" {...sortable} align="right" title="GPU framebuffer used / total across the pod's GPUs" />
            <SortableTh label="KV cache" sortKey="kvUsage" {...sortable} align="right" title="vLLM KV-cache blocks in use (max across the model's engines)" />
            <SortableTh label="Prefix hit" sortKey="prefixHit" {...sortable} align="right" title="Prefix-cache hits / queries over 5m" />
            <SortableTh label="Running" sortKey="running" {...sortable} align="right" />
            <SortableTh label="Waiting" sortKey="waiting" {...sortable} align="right" />
            <SortableTh label="Gen tok/s" sortKey="genTps" {...sortable} align="right" />
            <SortableTh label="TTFT p95" sortKey="ttftP95" {...sortable} align="right" />
            <SortableTh label="Tenants" sortKey="tenants" {...sortable} className="" title="LiteLLM teams with a virtual key routed to this model pool" />
          </tr>
        </thead>
        <tbody>
          {grouping === "model"
