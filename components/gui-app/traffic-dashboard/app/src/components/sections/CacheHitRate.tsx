...
export function CacheHitRate({ minutes }: { minutes: number }) {
  ...
  return (
    <PortletGrid id="cache">
      <Portlet id="tile-hit-ratio" span={3} label="Prefix-cache hit ratio">
        <StatTile
      ...
      <Portlet id="tile-cached-tokens" span={3} label="Cached prompt tokens">
        <StatTile
      ...
      <Portlet id="tile-kv-max" span={3} label="KV-cache usage (max)">
        <StatTile
      ...
      <Portlet id="tile-preemptions" span={3} label="Preemptions / min">
        <StatTile
      ...
      <Portlet id="hit-by-node" span={4} label="Hit ratio by worker node">
        <Card
      ...
      <Portlet id="hit-by-pool" span={4} label="Hit ratio by model pool">
        <Card
      ...
      <Portlet id="ttft-by-pool" span={4} label="TTFT p95 by model pool">
        <Card
      ...
      <Portlet id="prefill-per-request" span={4} label="Prefill tokens per request">
        <Card
      ...
      <Portlet id="prefill-by-source" span={4} label="Prefill tokens by source">
        <Card
      ...
      <Portlet id="tenant-cached-share" span={4} label="Cached prompt-token share by tenant">
        <Card
      ...
      <Portlet id="template-cached-share" span={4} label="Cached share by prompt template version">
        <Card
      ...
      <Portlet id="tenant-table" span={8} label="Tenant × template × pool">
        <Card
          title="Tenant × template × pool"
          subtitle="One row per LiteLLM key, end user, prompt template version and requested model pool. Sorted worst first; narrow by any identity column with the filters above the table."
        >
          <CacheTenantTable />
        </Card>
      </Portlet>

      <Portlet id="engine-diagnosis" span={12} label="Per-engine diagnosis">
        <Card
      ...
      <Portlet id="callers" span={12} label="Callers per model pool">
        <Card
