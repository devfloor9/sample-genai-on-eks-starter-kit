...
export function DeepLinks({ domain }: DeepLinksProps) {
  ...
  return (
    <PortletGrid id="links">
      {links.map((link) => (
        <Portlet key={link.title} id={link.title} span={4} minSpan={3} label={link.title}>
          <a
            href={link.href}
            target="_blank"
            rel="noreferrer noopener"
            className="group/link block h-full rounded-2xl bg-surface p-5 ring-1 ring-white/10 transition-colors hover:bg-surface-raised"
          >
            <div className="flex items-center justify-between gap-2">
              <h3 className="text-sm font-medium text-ink">{link.title}</h3>
              <span aria-hidden="true" className="text-xs text-ink-muted transition-colors group-hover/link:text-ink">
                ↗
              </span>
            </div>
