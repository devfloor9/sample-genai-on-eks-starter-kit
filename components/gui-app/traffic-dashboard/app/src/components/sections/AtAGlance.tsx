...
import { Portlet, PortletGrid, usePortletSized } from "../PortletGrid";
...
export function AtAGlance({ minutes }: { minutes: number }) {
  return (
    <PortletGrid id="at-a-glance">
      <Portlet id="cluster" span={12} label="Nodes & pods">
      ...
      <Portlet id="cpu" span={12} label="CPU">
      ...
      <Portlet id="memory" span={12} label="Memory">
      ...
      <Portlet id="network" span={12} label="Network">
      ...
      <Portlet id="storage" span={12} label="Storage">
      ...
      <Portlet id="accel" span={12} label="GPU & accelerators">
      ...
      <Portlet id="serving" span={12} label="Serving">
...
/**
 * One resource block. The tile grid is auto-fit so the block reflows to
 * however many columns its portlet spans: four tiles across at full width, a
 * 2×2 at half width.
 */
function Block({ index, title, tagline, children }: { index: string; title: string; tagline: string; children: React.ReactNode }) {
  ...
  return (
    <section aria-labelledby={`glance-${index}`} className="flex h-full min-h-0 flex-col pt-5">
      ...
      <div
        className={`grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-4 ${sized ? "min-h-0 flex-1 overflow-auto" : ""}`}
      >
        {children}
      </div>
    </section>
